...
select distinct Имя_Псевдоним from Исполнитель
    where Имя_Псевдоним not in (
        select distinct Имя_Псевдоним from Исполнитель
        left join Исполнитель_Альбом on Исполнитель.id = Исполнитель_Альбом.Исполнитель_id
        left join Альбом on Исполнитель_Альбом.Альбом_id = Альбом.id
        where Год_выпуска_альбома = 2020
        )
    order by Имя_Псевдоним
    ;
